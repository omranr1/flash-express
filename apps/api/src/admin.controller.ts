...
class AdminCreateCustomerDto { @IsString() name!: string; @IsString() @Matches(/^\+?[0-9 ]{8,20}$/) phone!: string }
@Controller('admin')
...
export class AdminController {
  ...
  @Post('customers') async createCustomer(@Body() body: AdminCreateCustomerDto) { const user = await this.prisma.user.upsert({ where: { phone: body.phone }, update: { name: body.name, isVerified: true, isActive: true }, create: { phone: body.phone, name: body.name, isVerified: true, customer: { create: { phone: body.phone } }, wallet: { create: {} } }, select: { id: true, name: true, phone: true, isActive: true, createdAt: true } }); await this.prisma.customer.upsert({ where: { userId: user.id }, update: { phone: body.phone }, create: { userId: user.id, phone: body.phone } }); await this.prisma.wallet.upsert({ where: { userId: user.id }, update: {}, create: { userId: user.id } }); return { success: true, message: 'تمت إضافة العميل', data: user } }
