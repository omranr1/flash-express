...
export class AuthController {
  ...
  @Post('logout') logout(@Res({ passthrough: true }) response: Response) { response.clearCookie('flash_access_token', { httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: process.env.NODE_ENV === 'production' ? 'none' : 'lax', path: '/' }); return { success: true, message: 'تم تسجيل الخروج', data: null } }
  @UseGuards(JwtGuard)
